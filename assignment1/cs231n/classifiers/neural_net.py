...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    z2 = a1.dot(W2)+b2
    scores = z2
    #scores = np.exp(a2)/np.sum(np.exp(a2), axis=1, keepdims=True)
    pass
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    #############################################################################
    #a1_loss = np.maximum(0, X.dot(W1)+b1)
    #a2_loss = a1_loss.dot(W2)+b2
    scores_loss = np.exp(scores)/np.sum(np.exp(scores), axis=1, keepdims=True)
    correct_scores_loss = scores_loss[xrange(scores_loss.shape[0]), y]
    loss = np.mean(-np.log(correct_scores_loss))+reg/2.0*(np.sum(W1*W1)+np.sum(W2*W2))
    pass
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    dcsl = -1.0/correct_scores_loss/correct_scores_loss.shape[0]
    
    mask = np.array(scores_loss)
    mask = mask*(-correct_scores_loss.reshape(-1,1))
    mask[xrange(mask.shape[0]), y] = correct_scores_loss*(1-correct_scores_loss)
    dscores = dcsl.reshape(-1,1)*mask

    dW2 = a1.T.dot(dscores) + reg*W2
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in xrange(num_iters):
      choice = np.random.choice(X.shape[0], batch_size)
      X_batch = X[choice]
      y_batch = y[choice]
